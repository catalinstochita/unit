...
#define NXT_CONF_VLDT_ANY_TYPE (NXT_CONF_VLDT_NULL | NXT_CONF_VLDT_BOOLEAN | NXT_CONF_VLDT_NUMBER | NXT_CONF_VLDT_STRING | NXT_CONF_VLDT_ARRAY | NXT_CONF_VLDT_OBJECT)
...
#define NXT_CONF_VLDT_NEXT(next) \
    {                            \
        .u.members = next        \
    }
#define NXT_CONF_VLDT_END       \
    {                           \
        .name = nxt_null_string \
    }
...
static nxt_conf_vldt_object_t nxt_conf_vldt_session_members[] = {
    ...
    },
    {
        ...
        .type = NXT_CONF_VLDT_STRING | NXT_CONF_VLDT_ARRAY | NXT_CONF_VLDT_BOOLEAN,
#if (NXT_HAVE_OPENSSL_TLSEXT)
                                   ...
                                   nxt_conf_value_t *value, void *data)
{
    ...
    if (nxt_strcasestr_eq(&scheme, &http) || nxt_strcasestr_eq(&scheme, &https))
    {
        return NXT_OK;
    }
                     ...
                     void *data)
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (vals->flags & NXT_CONF_VLDT_TSTR && nxt_conf_type(member) == NXT_CONF_STRING)
            {
